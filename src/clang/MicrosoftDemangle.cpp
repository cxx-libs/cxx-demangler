...
SpecialTableSymbolNode* Demangler::demangleSpecialTableSymbolNode( std::string_view& MangledName, SpecialIntrinsicKind K )
{
  ...
  while( !consumeFront( MangledName, '@' ) )
  {
    ...
    if( TargetCurrent ) TargetCurrent->Next = Next;
    else
      TargetHead = Next;
  ...
  if( Count > 0 ) STSN->TargetNames = nodeListToNodeArray( Arena, TargetHead, Count );
...
VariableSymbolNode* Demangler::demangleUntypedVariable( ArenaAllocator& Arena, std::string_view& MangledName, std::string_view VariableName )
{
...
VariableSymbolNode* Demangler::demangleRttiBaseClassDescriptorNode( ArenaAllocator& Arena, std::string_view& MangledName )
{
...
IdentifierNode* Demangler::demangleFunctionIdentifierCode( std::string_view& MangledName )
{
  ...
  if( consumeFront( MangledName, "__" ) ) return demangleFunctionIdentifierCode( MangledName, FunctionIdentifierCodeGroup::DoubleUnder );
  if( consumeFront( MangledName, "_" ) ) return demangleFunctionIdentifierCode( MangledName, FunctionIdentifierCodeGroup::Under );
...
IdentifierNode* Demangler::demangleFunctionIdentifierCode( std::string_view& MangledName, FunctionIdentifierCodeGroup Group )
{
  if( MangledName.empty() )
  {
    Error = true;
  ...
  switch( Group )
  {
    ...
    case FunctionIdentifierCodeGroup::Under: MangledName.remove_prefix( 1 ); return Arena.alloc<IntrinsicFunctionIdentifierNode>( translateIntrinsicFunctionCode( CH, Group ) );
...
EncodedStringLiteralNode* Demangler::demangleStringLiteral( std::string_view& MangledName )
{
  ...
  if( IsWcharT )
  {
    ...
    while( !consumeFront( MangledName, '@' ) )
    {
      ...
      if( StringByteSize % 2 != 0 ) goto StringLiteralError;
      if( StringByteSize == 0 ) goto StringLiteralError;
...
FuncClass Demangler::demangleFunctionClass( std::string_view& MangledName )
{
  ...
  switch( F )
  {
    ...
    case '$':
    {
      ...
      if( consumeFront( MangledName, 'R' ) ) VFlag = FuncClass( VFlag | FC_VirtualThisAdjustEx );
      if( MangledName.empty() ) break;
      const char F = MangledName.front();
      MangledName.remove_prefix( 1 );
      switch( F )
...
TypeNode* Demangler::demangleType( std::string_view& MangledName, QualifierMangleMode QMM )
{
  ...
  bool       IsMember = false;
  if( QMM == QualifierMangleMode::Mangle ) { std::tie( Quals, IsMember ) = demangleQualifiers( MangledName ); }
  ...
  else if( isCustomType( MangledName ) ) { Ty = demangleCustomType( MangledName ); }
  else
  {
...
std::optional<PointerAuthQualifierNode::ArgArray> Demangler::demanglePointerAuthQualifier( std::string_view& MangledName )
{
  if( !consumeFront( MangledName, "__ptrauth" ) ) return std::nullopt;
  ...
  for( unsigned I = 0; I < NumArgs; ++I )
  {
    ...
    if( IsNegative ) return std::nullopt;
...
PointerAuthQualifierNode* Demangler::createPointerAuthQualifier( std::string_view& MangledName )
{
  constexpr unsigned                                NumArgs = PointerAuthQualifierNode::NumArgs;
  std::optional<PointerAuthQualifierNode::ArgArray> Vals    = demanglePointerAuthQualifier( MangledName );
  ...
  for( unsigned I = 0; I < NumArgs; ++I ) Array->Nodes[I] = Arena.alloc<IntegerLiteralNode>( ( *Vals )[I], false );
...
NodeArrayNode* Demangler::demangleFunctionParameterList( std::string_view& MangledName, bool& IsVariadic )
{
  ...
  while( !Error && !cxx::demangler::backend::clang::starts_with( MangledName, '@' ) && !cxx::demangler::backend::clang::starts_with( MangledName, 'Z' ) )
  {
    ...
    *Current     = Arena.alloc<NodeList>();
    ...
    if( Backrefs.FunctionParamCount <= 9 && CharsConsumed > 1 ) Backrefs.FunctionParams[Backrefs.FunctionParamCount++] = TN;
...
NodeArrayNode* Demangler::demangleTemplateParameterList( std::string_view& MangledName )
{
  ...
  while( !cxx::demangler::backend::clang::starts_with( MangledName, '@' ) )
  {
    ...
    *Current = Arena.alloc<NodeList>();
    ...
    else if( startsWith( MangledName, "$1", "1", !IsAutoNTTP ) || startsWith( MangledName, "$H", "H", !IsAutoNTTP ) || startsWith( MangledName, "$I", "I", !IsAutoNTTP ) || startsWith( MangledName, "$J", "J", !IsAutoNTTP ) )
    {
      // Pointer to member
...
std::optional<std::size_t> cxx::demangler::backend::clang::getArm64ECInsertionPointInMangledName( const std::string_view MangledName )
{
  ...
  if( !consumeFront( ProcessedName, '?' ) ) return std::nullopt;
