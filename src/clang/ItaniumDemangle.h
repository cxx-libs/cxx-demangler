...
{
  static_assert( std::is_trivially_copyable<T>::value, "T is required to be a trivially copyable type" );
  static_assert( std::is_trivially_default_constructible<T>::value, "T is required to be trivially default constructible" );
  T* First     = nullptr;
  T* Last      = nullptr;
  T* Cap       = nullptr;
  T  Inline[N] = {};
...
public:
  Node( Kind K_, Prec Precedence_ = Prec::Primary, Cache RHSComponentCache_ = Cache::No, Cache ArrayCache_ = Cache::No, Cache FunctionCache_ = Cache::No ) :
    K( K_ ), Precedence( Precedence_ ), RHSComponentCache( RHSComponentCache_ ), ArrayCache( ArrayCache_ ), FunctionCache( FunctionCache_ )
  {
  }
  Node( Kind K_, Cache RHSComponentCache_, Cache ArrayCache_ = Cache::No, Cache FunctionCache_ = Cache::No ) : Node( K_, Prec::Primary, RHSComponentCache_, ArrayCache_, FunctionCache_ ) {}
  ...
  {
    OB.printLeft( *this );
    if( RHSComponentCache != Cache::No ) OB.printRight( *this );
  }
...
public:
  VendorExtQualType( const Node* Ty_, std::string_view Ext_, const Node* TA_ ) : Node( KVendorExtQualType ), Ty( Ty_ ), Ext( Ext_ ), TA( TA_ ) {}
...
public:
  QualType( const Node* Child_, Qualifiers Quals_ ) : Node( KQualType, Child_->getRHSComponentCache(), Child_->getArrayCache(), Child_->getFunctionCache() ), Quals( Quals_ ), Child( Child_ ) {}
...
public:
  PostfixQualifiedType( const Node* Ty_, std::string_view Postfix_ ) : Node( KPostfixQualifiedType ), Ty( Ty_ ), Postfix( Postfix_ ) {}
...
public:
  ElaboratedTypeSpefType( std::string_view Kind_, Node* Child_ ) : Node( KElaboratedTypeSpefType ), Kind( Kind_ ), Child( Child_ ) {}
...
public:
  TransformedType( std::string_view Transform_, Node* BaseType_ ) : Node( KTransformedType ), Transform( Transform_ ), BaseType( BaseType_ ) {}
...
struct AbiTagAttr : Node
{
  ...
  AbiTagAttr( Node* Base_, std::string_view Tag_ ) : Node( KAbiTagAttr, Base_->getRHSComponentCache(), Base_->getArrayCache(), Base_->getFunctionCache() ), Base( Base_ ), Tag( Tag_ ) {}
...
public:
  ReferenceType( const Node* Pointee_, ReferenceKind RK_ ) : Node( KReferenceType, Pointee_->getRHSComponentCache() ), Pointee( Pointee_ ), RK( RK_ ) {}
...
public:
  PointerToMemberType( const Node* ClassType_, const Node* MemberType_ ) : Node( KPointerToMemberType, MemberType_->getRHSComponentCache() ), ClassType( ClassType_ ), MemberType( MemberType_ ) {}
...
public:
  ...
  {
    if( Base->getKind() == KNameType && static_cast<const NameType*>( Base )->getName() == "char" ) { return Elements.printAsString( OB ); }
    return false;
  }
};
...
public:
  FunctionEncoding( const Node* Ret_, const Node* Name_, NodeArray Params_, const Node* Attrs_, const Node* Requires_, Qualifiers CVQuals_, FunctionRefQual RefQual_ ) :
    Node( KFunctionEncoding,
          /*RHSComponentCache=*/Cache::Yes, /*ArrayCache=*/Cache::No,
          /*FunctionCache=*/Cache::Yes ),
    Ret( Ret_ ), Name( Name_ ), Params( Params_ ), Attrs( Attrs_ ), Requires( Requires_ ), CVQuals( CVQuals_ ), RefQual( RefQual_ )
  {
  }

  template<typename Fn> void match( Fn F ) const { F( Ret, Name, Params, Attrs, Requires, CVQuals, RefQual ); }
...
struct ModuleName : Node
{
  ...
  ModuleName( ModuleName* Parent_, Node* Name_, bool IsPartition_ = false ) : Node( KModuleName ), Parent( Parent_ ), Name( Name_ ), IsPartition( IsPartition_ ) {}

  template<typename Fn> void match( Fn F ) const { F( Parent, Name, IsPartition ); }
...
public:
  ...
  void printLeft( OutputBuffer& OB ) const override
  {
    switch( Kind )
    {
      case TemplateParamKind::Type: OB += "$T"; break;
      case TemplateParamKind::NonType: OB += "$N"; break;
      case TemplateParamKind::Template: OB += "$TT"; break;
    }
...
public:
  ConstrainedTypeTemplateParamDecl( Node* Constraint_, Node* Name_ ) : Node( KConstrainedTypeTemplateParamDecl, Cache::Yes ), Constraint( Constraint_ ), Name( Name_ ) {}
...
public:
  TemplateTemplateParamDecl( Node* Name_, NodeArray Params_, Node* Requires_ ) : Node( KTemplateTemplateParamDecl, Cache::Yes ), Name( Name_ ), Params( Params_ ), Requires( Requires_ ) {}
...
public:
  ParameterPack( NodeArray Data_ ) : Node( KParameterPack ), Data( Data_ )
  {
    ...
    if( std::all_of( Data.begin(), Data.end(), []( Node* P ) { return P->getArrayCache() == Cache::No; } ) ) ArrayCache = Cache::No;
    if( std::all_of( Data.begin(), Data.end(), []( Node* P ) { return P->getFunctionCache() == Cache::No; } ) ) FunctionCache = Cache::No;
    if( std::all_of( Data.begin(), Data.end(), []( Node* P ) { return P->getRHSComponentCache() == Cache::No; } ) ) RHSComponentCache = Cache::No;
  }
  ...
  void printLeft( OutputBuffer& OB ) const override
  {
    initializePackExpansion( OB );
    size_t Idx = OB.CurrentPackIndex;
    if( Idx < Data.size() ) OB.printLeft( *Data[Idx] );
  }
  void printRight( OutputBuffer& OB ) const override
  {
    initializePackExpansion( OB );
    size_t Idx = OB.CurrentPackIndex;
    if( Idx < Data.size() ) OB.printRight( *Data[Idx] );
  }
};
...
public:
  ExpandedSpecialSubstitution( SpecialSubKind SSK_ ) : ExpandedSpecialSubstitution( SSK_, KExpandedSpecialSubstitution ) {}
...
inline ExpandedSpecialSubstitution::ExpandedSpecialSubstitution( SpecialSubstitution const* SS ) : ExpandedSpecialSubstitution( SS->SSK ) {}
...
public:
  ClosureTypeName( NodeArray TemplateParams_, const Node* Requires1_, NodeArray Params_, const Node* Requires2_, std::string_view Count_ ) :
    Node( KClosureTypeName ), TemplateParams( TemplateParams_ ), Requires1( Requires1_ ), Params( Params_ ), Requires2( Requires2_ ), Count( Count_ )
  {
  }

  template<typename Fn> void match( Fn F ) const { F( TemplateParams, Requires1, Params, Requires2, Count ); }
...
public:
  BinaryExpr( const Node* LHS_, std::string_view InfixOperator_, const Node* RHS_, Prec Prec_ ) : Node( KBinaryExpr, Prec_ ), LHS( LHS_ ), InfixOperator( InfixOperator_ ), RHS( RHS_ ) {}

  template<typename Fn> void match( Fn F ) const { F( LHS, InfixOperator, RHS, getPrecedence() ); }

  void printLeft( OutputBuffer& OB ) const override
  {
    // If we're printing a '<' inside of a template argument, and we haven't
    // yet parenthesized the expression, do so now.
    bool ParenAll = !OB.isInParensInTemplateArgs() && ( InfixOperator == ">" || InfixOperator == ">>" );
...
public:
  ArraySubscriptExpr( const Node* Op1_, const Node* Op2_, Prec Prec_ ) : Node( KArraySubscriptExpr, Prec_ ), Op1( Op1_ ), Op2( Op2_ ) {}

  template<typename Fn> void match( Fn F ) const { F( Op1, Op2, getPrecedence() ); }
...
public:
  PostfixExpr( const Node* Child_, std::string_view Operator_, Prec Prec_ ) : Node( KPostfixExpr, Prec_ ), Child( Child_ ), Operator( Operator_ ) {}

  template<typename Fn> void match( Fn F ) const { F( Child, Operator, getPrecedence() ); }
...
public:
  ConditionalExpr( const Node* Cond_, const Node* Then_, const Node* Else_, Prec Prec_ ) : Node( KConditionalExpr, Prec_ ), Cond( Cond_ ), Then( Then_ ), Else( Else_ ) {}

  template<typename Fn> void match( Fn F ) const { F( Cond, Then, Else, getPrecedence() ); }
...
public:
  MemberExpr( const Node* LHS_, std::string_view Kind_, const Node* RHS_, Prec Prec_ ) : Node( KMemberExpr, Prec_ ), LHS( LHS_ ), Kind( Kind_ ), RHS( RHS_ ) {}

  template<typename Fn> void match( Fn F ) const { F( LHS, Kind, RHS, getPrecedence() ); }
...
public:
  SubobjectExpr( const Node* Type_, const Node* SubExpr_, std::string_view Offset_, NodeArray UnionSelectors_, bool OnePastTheEnd_ ) :
    Node( KSubobjectExpr ), Type( Type_ ), SubExpr( SubExpr_ ), Offset( Offset_ ), UnionSelectors( UnionSelectors_ ), OnePastTheEnd( OnePastTheEnd_ )
  {
  }

  template<typename Fn> void match( Fn F ) const { F( Type, SubExpr, Offset, UnionSelectors, OnePastTheEnd ); }
...
public:
  EnclosingExpr( std::string_view Prefix_, const Node* Infix_, Prec Prec_ = Prec::Primary ) : Node( KEnclosingExpr, Prec_ ), Prefix( Prefix_ ), Infix( Infix_ ) {}

  template<typename Fn> void match( Fn F ) const { F( Prefix, Infix, getPrecedence() ); }
...
public:
  CastExpr( std::string_view CastKind_, const Node* To_, const Node* From_, Prec Prec_ ) : Node( KCastExpr, Prec_ ), CastKind( CastKind_ ), To( To_ ), From( From_ ) {}

  template<typename Fn> void match( Fn F ) const { F( CastKind, To, From, getPrecedence() ); }
...
public:
  CallExpr( const Node* Callee_, NodeArray Args_, bool IsParen_, Prec Prec_ ) : Node( KCallExpr, Prec_ ), Callee( Callee_ ), Args( Args_ ), IsParen( IsParen_ ) {}

  template<typename Fn> void match( Fn F ) const { F( Callee, Args, IsParen, getPrecedence() ); }

  void printLeft( OutputBuffer& OB ) const override
  {
    if( IsParen ) OB.printOpen();
    Callee->print( OB );
    if( IsParen ) OB.printClose();
    OB.printOpen();
...
class NewExpr : public Node
{
  ...
  bool      IsArray;   // new[] ?
public:
  NewExpr( NodeArray ExprList_, Node* Type_, NodeArray InitList_, bool IsGlobal_, bool IsArray_, Prec Prec_ ) : Node( KNewExpr, Prec_ ), ExprList( ExprList_ ), Type( Type_ ), InitList( InitList_ ), IsGlobal( IsGlobal_ ), IsArray( IsArray_ ) {}

  template<typename Fn> void match( Fn F ) const { F( ExprList, Type, InitList, IsGlobal, IsArray, getPrecedence() ); }
...
public:
  DeleteExpr( Node* Op_, bool IsGlobal_, bool IsArray_, Prec Prec_ ) : Node( KDeleteExpr, Prec_ ), Op( Op_ ), IsGlobal( IsGlobal_ ), IsArray( IsArray_ ) {}

  template<typename Fn> void match( Fn F ) const { F( Op, IsGlobal, IsArray, getPrecedence() ); }
...
public:
  ...
  template<typename Fn> void match( Fn F ) const { F( Prefix, Child, getPrecedence() ); }
...
public:
  ConversionExpr( const Node* Type_, NodeArray Expressions_, Prec Prec_ ) : Node( KConversionExpr, Prec_ ), Type( Type_ ), Expressions( Expressions_ ) {}

  template<typename Fn> void match( Fn F ) const { F( Type, Expressions, getPrecedence() ); }
...
public:
  PointerToMemberConversionExpr( const Node* Type_, const Node* SubExpr_, std::string_view Offset_, Prec Prec_ ) : Node( KPointerToMemberConversionExpr, Prec_ ), Type( Type_ ), SubExpr( SubExpr_ ), Offset( Offset_ ) {}

  template<typename Fn> void match( Fn F ) const { F( Type, SubExpr, Offset, getPrecedence() ); }
...
public:
  FoldExpr( bool IsLeftFold_, std::string_view OperatorName_, const Node* Pack_, const Node* Init_ ) : Node( KFoldExpr ), Pack( Pack_ ), Init( Init_ ), OperatorName( OperatorName_ ), IsLeftFold( IsLeftFold_ ) {}
...
public:
  ...
  void printLeft( OutputBuffer& OB ) const override { OB += Value ? std::string_view( "true" ) : std::string_view( "false" ); }
};
...
public:
  IntegerLiteral( std::string_view Type_, std::string_view Value_ ) : Node( KIntegerLiteral ), Type( Type_ ), Value( Value_ ) {}
...
public:
  RequiresExpr( NodeArray Parameters_, NodeArray Requirements_ ) : Node( KRequiresExpr ), Parameters( Parameters_ ), Requirements( Requirements_ ) {}
  ...
  void printLeft( OutputBuffer& OB ) const override
  {
    ...
    OB += ' ';
    OB.printOpen( '{' );
    for( const Node* Req: Requirements ) { Req->print( OB ); }
    OB += ' ';
    OB.printClose( '}' );
  }
};
...
public:
  ExprRequirement( const Node* Expr_, bool IsNoexcept_, const Node* TypeConstraint_ ) : Node( KExprRequirement ), Expr( Expr_ ), IsNoexcept( IsNoexcept_ ), TypeConstraint( TypeConstraint_ ) {}

  template<typename Fn> void match( Fn F ) const { F( Expr, IsNoexcept, TypeConstraint ); }
...
inline bool NodeArray::printAsString( OutputBuffer& OB ) const
{
  ...
  for( const Node* Element: *this )
  {
    ...
    {
      if( ( integer_value >= '0' && integer_value <= '9' ) || ( integer_value >= 'a' && integer_value <= 'f' ) || ( integer_value >= 'A' && integer_value <= 'F' ) ) { OB += "\"\""; }
    }
...
template<typename Derived, typename Alloc> struct AbstractManglingParser
{
  ...
  template<class T, class... Args> Node* make( Args&&... args ) { return ASTAllocator.template makeNode<T>( std::forward<Args>( args )... ); }
  ...
  bool isTemplateParamDecl() { return look() == 'T' && std::string_view( "yptnk" ).find( look( 1 ) ) != std::string_view::npos; }
  ...
  public:
    constexpr OperatorInfo( const char ( &E )[3], OIKind K, bool F, Node::Prec P, const char* N ) : Enc{ E[0], E[1] }, Kind{ K }, Flag{ F }, Prec{ P }, Name{ N } {}
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseLocalName( NameState* State )
{
...
// [*] extension
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseUnscopedName( NameState* State, bool* IsSubst )
{
  Node* Std = nullptr;
  if( consumeIf( "St" ) )
  {
  ...
  if( Res == nullptr || Std != nullptr ) { Res = getDerived().parseUnqualifiedName( State, Std, Module ); }
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseUnqualifiedName( NameState* State, Node* Scope, ModuleName* Module )
{
  ...
  Node* Result;
  if( look() >= '1' && look() <= '9' ) { Result = getDerived().parseSourceName( State ); }
  else if( look() == 'U' ) { Result = getDerived().parseUnnamedTypeName( State ); }
...
//		    ::= W P <source-name>
template<typename Derived, typename Alloc> bool AbstractManglingParser<Derived, Alloc>::parseModuleNameOpt( ModuleName*& Module )
{
  while( consumeIf( 'W' ) )
  {
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseUnnamedTypeName( NameState* State )
{
...
const typename AbstractManglingParser<Derived, Alloc>::OperatorInfo AbstractManglingParser<Derived, Alloc>::Ops[] = {
  ...
  { "pm", OperatorInfo::Member, /*Named*/ true, Node::Prec::PtrMem, "operator->*" },
...
template<typename Derived, typename Alloc> const size_t AbstractManglingParser<Derived, Alloc>::NumOps = sizeof( Ops ) / sizeof( Ops[0] );

// If the next 2 chars are an operator encoding, consume them and return their
// OperatorInfo.  Otherwise return nullptr.
template<typename Derived, typename Alloc> const typename AbstractManglingParser<Derived, Alloc>::OperatorInfo* AbstractManglingParser<Derived, Alloc>::parseOperatorEncoding()
{
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseOperatorName( NameState* State )
{
  if( const auto* Op = parseOperatorEncoding() )
  {
    if( Op->getKind() == OperatorInfo::CCast )
    {
      ...
      ScopedOverride<bool> SavePermit( PermitForwardTemplateReferences, PermitForwardTemplateReferences || State != nullptr );
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseCtorDtorName( Node*& SoFar, NameState* State )
{
...
//                   ::= <template-param>
//                   ::= <substitution>
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseNestedName( NameState* State )
{
  ...
  else if( State ) { State->HasExplicitObjectParameter = true; }
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseUnresolvedName( bool Global )
{
  Node* SoFar = nullptr;
...
template<typename Alloc, typename Derived> std::string_view AbstractManglingParser<Alloc, Derived>::parseNumber( bool AllowNegative )
{
...
template<typename Alloc, typename Derived> bool AbstractManglingParser<Alloc, Derived>::parsePositiveInteger( size_t* Out )
{
  *Out = 0;
...
template<typename Alloc, typename Derived> std::string_view AbstractManglingParser<Alloc, Derived>::parseBareSourceName()
{
  size_t Int = 0;
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseQualifiedType()
{
  if( consumeIf( 'U' ) )
  {
    ...
    if( cxx::demangler::backend::clang::starts_with( Qual, "objcproto" ) )
    {
      ...
      {
        ScopedOverride<const char*> SaveFirst( First, ProtoSourceName.data() ), SaveLast( Last, &*ProtoSourceName.rbegin() + 1 );
  ...
  switch( look() )
  {
    ...
    case 'D':
      switch( look( 1 ) )
      {
        ...
        case 'h': First += 2; return make<NameType>( "half" );
        //       ::= DF16b         # C++23 std::bfloat16_t
        //       ::= DF <number> _ # ISO/IEC TS 18661 binary floating point (N bits)
        case 'F':
        {
          First += 2;
          if( consumeIf( "16b" ) ) return make<NameType>( "std::bfloat16_t" );
          Node* DimensionNumber = make<NameType>( parseNumber() );
          if( !DimensionNumber ) return nullptr;
          if( !consumeIf( '_' ) ) return nullptr;
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parsePrefixExpr( std::string_view Kind, Node::Prec Prec )
{
  Node* E = getDerived().parseExpr();
  if( E == nullptr ) return nullptr;
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseBinaryExpr( std::string_view Kind, Node::Prec Prec )
{
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseIntegerLiteral( std::string_view Lit )
{
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseFoldExpr()
{
  ...
  switch( look() )
  {
    ...
    case 'L':
      ...
      HasInitializer = true;
      break;
    case 'R': HasInitializer = true; break;
    case 'l': IsLeftFold = true; break;
    case 'r': break;
  }
  ++First;
  ...
  if( !( Op->getKind() == OperatorInfo::Binary || ( Op->getKind() == OperatorInfo::Member && Op->getName().back() == '*' ) ) ) return nullptr;
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parsePointerToMemberConversionExpr( Node::Prec Prec )
{
  Node* Ty = getDerived().parseType();
...
//
// Not yet in the spec: https://github.com/itanium-cxx-abi/cxx-abi/issues/47
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseSubobjectExpr()
{
  Node* Ty = getDerived().parseType();
  if( !Ty ) return nullptr;
  Node* Expr = getDerived().parseExpr();
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseConstraintExpr()
{
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseRequiresExpr()
{
  NodeArray Params;
  if( consumeIf( "rQ" ) )
  {
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseExpr()
{
  ...
  if( Op )
  {
    ...
    switch( Op->getKind() )
    {
      ...
      case OperatorInfo::Call:
      {
        ...
        return make<CallExpr>( Callee, popTrailingNodeArray( ExprsBegin ), /*IsParen=*/Op->getFlag(), Op->getPrecedence() );
      }
  ...
  if( consumeIf( 'u' ) )
  {
    ...
    return make<CallExpr>( Name, popTrailingNodeArray( ExprsBegin ), /*IsParen=*/false, Node::Prec::Postfix );
  }
...
template<typename Alloc, typename Derived> bool AbstractManglingParser<Alloc, Derived>::parseCallOffset()
{
  ...
  if( consumeIf( 'v' ) ) return parseNumber( true ).empty() || !consumeIf( '_' ) || parseNumber( true ).empty() || !consumeIf( '_' );
  return true;
}
...
//            ::= <data name>
//            ::= <special-name>
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseEncoding( bool ParseParams )
{
  ...
  return make<FunctionEncoding>( ReturnType, Name, Params, Attrs, Requires, NameInfo.CVQualifiers, NameInfo.ReferenceQualifier );
}
...
template<typename Alloc, typename Derived> template<class Float> Node* AbstractManglingParser<Alloc, Derived>::parseFloatingLiteral()
{
  const size_t N = FloatData<Float>::mangled_size;
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseSubstitution()
{
  ...
  if( look() >= 'a' && look() <= 'z' )
  {
    ...
    switch( look() )
    {
      case 'a': Kind = SpecialSubKind::allocator; break;
      case 'b': Kind = SpecialSubKind::basic_string; break;
      case 'd': Kind = SpecialSubKind::iostream; break;
      case 'i': Kind = SpecialSubKind::istream; break;
      case 'o': Kind = SpecialSubKind::ostream; break;
      case 's': Kind = SpecialSubKind::string; break;
      default: return nullptr;
    }
    ++First;
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseTemplateParam()
{
  ...
  if( HasIncompleteTemplateParameterTracking ) { return make<NameType>( std::string_view( Begin, First - 1 - Begin ) ); }
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseTemplateParamDecl( TemplateParamList* Params )
{
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parseTemplateArgs( bool TagTemplates )
{
  ...
  while( !consumeIf( 'E' ) )
  {
    if( TagTemplates )
    {
      ...
      Names.push_back( Arg );
      Node* TableEntry = Arg;
      if( Arg->getKind() == Node::KTemplateParamQualifiedArg ) { TableEntry = static_cast<TemplateParamQualifiedArg*>( TableEntry )->getArg(); }
...
template<typename Derived, typename Alloc> Node* AbstractManglingParser<Derived, Alloc>::parse( bool ParseParams )
{
